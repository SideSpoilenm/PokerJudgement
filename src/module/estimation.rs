use super::super::range::co::CO_RANGE;
use super::card::{Card, Rank, Suit};
use super::{combination, judgement, convert};
...
#[allow(dead_code)]
pub fn calc(flop_cards: &[Card], hero_cards: &[Card]) {
    ...
    for vill_cards in &hands {
        ...
        let boardmap = combination::boardmap(
            &flop_cards,
            &[ Card { suit: Suit::default(), rank: Rank::default() }, Card { suit: Suit::default(), rank: Rank::default(),},],
            &vill_cards
        );
    ...
    println!("*************** ESTIMATION ***************");
    // for i in (0..HR.len()).rev() {
    //     println!("{}: {}%", HR[i], (vill_prop[i] * 100.0) as f32);
    // }
...
fn anlysis (hands: &[Vec<Card>], abilities: &[[f64; HR.len()]]){
    ...
    for i in 0..hands.len(){
        ...
        if flush_ability > 0.16{    // フラッシュドロー 36%
            flush_hands.push(hands[i].to_vec());
        }else if straight_ability > 0.16{ // ガットショットストレートドロー 16%
            straight_hands.push(hands[i].to_vec());
        }else if hit_ability > 0.08{  // ワンペアがセットになる   8.4%
    ...
    let flushhands_label: Vec<String> = flush_hands.iter().map(|hand| convert::hand_to_label(hand)).collect();
    let straighthands_label: Vec<String> = straight_hands.iter().map(|hand| convert::hand_to_label(hand)).collect();
    let hithands_label: Vec<String> = hit_hands.iter().map(|hand| convert::hand_to_label(hand)).collect();
    let otherhands_label: Vec<String> = other_hands.iter().map(|hand| convert::hand_to_label(hand)).collect();
